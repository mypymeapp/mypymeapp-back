...
export class CategoriesService {
    ...
    async create(data: CreateCategoryDto) {
        ...
        if (!company) {
            throw new BadRequestException(`Compañia con id ${data.companyId} no existe`);
        }
        ...
        if (exists) {
            throw new BadRequestException( `Categoria '${data.name}' ya existe en esta compañia`);
        }
    ...
    async remove(id: string) {
        ...
        if (hasProducts) {
            throw new BadRequestException(`No se pueden borrar categorias con productos asociados`);
        }
